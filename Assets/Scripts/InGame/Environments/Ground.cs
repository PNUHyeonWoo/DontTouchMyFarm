...
public class Ground : MonoBehaviour
{
    ...
    public int[] IfCanInstallSelectStructGetStartIndex(Vector2 rayPos) //설치 할 수 있으면 시작 인덱스를, 없으면 -1을 담은 배열을 리턴 
    {
    ...
    public bool InstallSelectStruct(int[] startIndex) //구조물 오브젝트 생성 및 installGrid 갱신, 실패 시 false리턴
    {
        int structsize = StructInterface.GetStructInterface().Size;
        if (IsCanInstall(startIndex, structsize))
        {
    ...
    public void DestroyStruct(StructInterface target) // 구조물 오브젝트 파괴 및 installGrid 갱신
    {
